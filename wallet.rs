...
pub fn fetch_wallet_by_id(_address: Uuid, conn: &DBPooledConnection) -> Option<Wallet> {
    ...
    .load::<WalletDAO>(conn) {
        Ok(result) => match result.first() {
            Some(matched_wallets_dao) => {
                match miners
                .filter(crate::schema::miners::address.eq(_address))
                .load::<MinerDAO>(conn) {
                    Ok(miner_result) => Some(matched_wallets_dao.to_wallet (
                        miner_result.into_iter().map(|w| {
                        m.to_miner(matched_wallets_dao.club_name.clone())
                    }).collect::<Vec<Miner>>())),
                        Err(_) => Some(matched_wallets_dao.to_wallet(vec![])),
                }
            },
            _ => return None,
        },
...
pub fn create_new_wallet(new_wallet_request: NewWalletRequest, conn: &DBPooledConnection) -> Result<Wallet, Error> {
    ...
    match diesel::insert_into(wallets).values(&new_wallet_dao).excute(conn) {
